DROP TABLE review;
DROP TABLE shipment;
DROP TABLE productinventory;
DROP TABLE warehouse;
DROP TABLE orderproduct;
DROP TABLE incart;
DROP TABLE product;
DROP TABLE category;
DROP TABLE ordersummary;
DROP TABLE paymentmethod;
DROP TABLE customer;


CREATE TABLE customer (
    customerId          INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE paymentmethod (
    paymentMethodId     INT IDENTITY,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE ordersummary (
    orderId             INT IDENTITY,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE category (
    categoryId          INT IDENTITY,
    categoryName        VARCHAR(50),    
    PRIMARY KEY (categoryId)
);

CREATE TABLE product (
    productId           INT IDENTITY,
    productName         VARCHAR(40),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        VARBINARY(MAX),
    productDesc         VARCHAR(1000),
    categoryId          INT,
    PRIMARY KEY (productId),
    FOREIGN KEY (categoryId) REFERENCES category(categoryId)
);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE warehouse (
    warehouseId         INT IDENTITY,
    warehouseName       VARCHAR(30),    
    PRIMARY KEY (warehouseId)
);

CREATE TABLE shipment (
    shipmentId          INT IDENTITY,
    shipmentDate        DATETIME,   
    shipmentDesc        VARCHAR(100),   
    warehouseId         INT, 
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory ( 
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (productId, warehouseId),   
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE review (
    reviewId            INT IDENTITY,
    reviewRating        INT,
    reviewDate          DATETIME,   
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),          
    PRIMARY KEY (reviewId),
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

/* regular gems
 * video game gems
 * probably legal gems
 * hippie gems
 * banana
 * internet gems
 * jims 
 */
INSERT INTO category(categoryName) VALUES ('Gem Stones');
INSERT INTO category(categoryName) VALUES ('Vidya Game Gems');
INSERT INTO category(categoryName) VALUES ('Probably Legal');
INSERT INTO category(categoryName) VALUES ('Hippie Gems');
INSERT INTO category(categoryName) VALUES ('Pickles');
INSERT INTO category(categoryName) VALUES ('Crypto Gems');
--INSERT INTO category(categoryName) VALUES ('Jims');
--INSERT INTO category(categoryName) VALUES ('Grains/Cereals');


INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Diamond', 1, 'x10 - Super Real Diamonds.',18.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Ruby',1,'x24 - Super Real Rubies.',19.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Amethyst',1,'x3.5 - Super Real Amethysts.',10.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Diamond Block',2,'x1 - Screw digging - Buy em by the block here! ',22.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Chaos Emerald',2,'x1 - Yenno, the sonic ones.',21.35);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Mario Coin',2,'x1 - Buy 100 - get a free life!',25.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Blood Diamonds',3,'x1 - "Ethically" Sourced, by my boy Leo, of course.',30.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Crystal Meth',3,'x1 - Sell drugs like the cool kids.',40.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Crystal Skull',3,'x1 - Contact aliens and stuff!',97.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Crystal Ball',4,'x1 - Totally tells the future. Man.',31.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('BitCoin',6,'x0.002 - BTC - Could be worth 20k or 20 bucks.',21.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Ethereum',6,'x10 - ETH - Etheruem Foundation Goodness',38.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Ripple',6,'x34 - XRP - Will go up. Tomorrow.',23.25);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Topaz',1,'x3 - The yellow one I think!',15.50);
--INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Jimi Hendrix',7,'x1 - Jim"s a gem.',17.45);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('PokeMon Ruby',2,'x1 - Sort of a gem.',39.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Chakra Crystals',4,'x5 - My mom says it aligns your chakras. lol.',62.50);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Jar of Pickles',5,'x1 - Green Gold.',9.20);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Labradorite',1,'x1 - Cutest of all gems.',81.00);

    
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Arnold', 'Anderson', 'a.anderson@gmail.com', '204-111-2222', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'arnold' , 'test');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Bobby', 'Brown', 'bobby.brown@hotmail.ca', '572-342-8911', '222 Bush Avenue', 'Boston', 'MA', '22222', 'United States', 'bobby' , 'bobby');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Candace', 'Cole', 'cole@charity.org', '333-444-5555', '333 Central Crescent', 'Chicago', 'IL', '33333', 'United States', 'candace' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Darren', 'Doe', 'oe@doe.com', '250-807-2222', '444 Dover Lane', 'Kelowna', 'BC', 'V1V 2X9', 'Canada', 'darren' , 'pw');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Elizabeth', 'Elliott', 'engel@uiowa.edu', '555-666-7777', '555 Everwood Street', 'Iowa City', 'IA', '52241', 'United States', 'beth' , 'test');



-- New SQL DDL for lab 8
UPDATE Product SET productImageURL = 'img/diamond.jpg' WHERE ProductId = 1;
UPDATE Product SET productImageURL = 'img/ruby.jpg' WHERE ProductId = 2;
UPDATE Product SET productImageURL = 'img/Amethyst.jpg' WHERE ProductId = 3;
UPDATE Product SET productImageURL = 'img/mcraftdiamond.jpg' WHERE ProductId = 4;
UPDATE Product SET productImageURL = 'img/chaosEmerald.jpg' WHERE ProductId = 5;
UPDATE Product SET productImageURL = 'img/mariocoin.jpg' WHERE ProductId = 6;
UPDATE Product SET productImageURL = 'img/blooddiamond.jpg' WHERE ProductId = 7;
UPDATE Product SET productImageURL = 'img/crystalmath.jpg' WHERE ProductId = 8;
UPDATE Product SET productImageURL = 'img/crystalskull.jpg' WHERE ProductId = 9;
UPDATE Product SET productImageURL = 'img/crystalball.jpg' WHERE ProductId = 10;
UPDATE Product SET productImageURL = 'img/bitcoin.jpg' WHERE ProductId =11;
UPDATE Product SET productImageURL = 'img/ethereum.jpg' WHERE ProductId = 12;
UPDATE Product SET productImageURL = 'img/ripple.jpg' WHERE ProductId = 13;
UPDATE Product SET productImageURL = 'img/topaz.jpg' WHERE ProductId = 14;
UPDATE Product SET productImageURL = 'img/pkmruby.jpg' WHERE ProductId = 15;
UPDATE Product SET productImageURL = 'img/chakra.jpg' WHERE ProductId = 16;
UPDATE Product SET productImageURL = 'img/pickles.jpg' WHERE ProductId = 17;
UPDATE Product SET productImageURL = 'img/labradorite.jpg' WHERE ProductId = 18;


-- Loads image data for product 1
UPDATE Product SET productImage =  WHERE ProductId = 1;







